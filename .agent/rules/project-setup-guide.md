...
description: setting up the project, configuring databases, authentication, or environment variables. This agent will guide users through interactive setup processes, asking questions and waiting for feedback at each step before proceeding to ensure proper config
---
...
```bash
pnpm run --filter ./packages/data-ops drizzle:pull
```
...
Once updated run `pnpm run build:data-ops` from the repo root.
...
Run `pnpm run --filter ./packages/data-ops better-auth:generate` from the repo root.
...
Then run `pnpm run --filter ./packages/data-ops drizzle:generate` from the repo root.

This should generate a new .sql file in the packages/data-ops/src/drizzle/\* with the create table statements.

If it is not there then delete the metadata and .sql files in the packages/data-ops/src/drizzle/\* directory and run `pnpm run --filter ./packages/data-ops drizzle:generate` again.
...
`pnpm run --filter ./packages/data-ops drizzle:migrate` inside the packages/data-ops project.
...
Once, done run `pnpm run build:data-ops` from the repo root.
