...
function RouteComponent() {
  ...
  const [results, setResults] = useState<{
    clientError?: { type: 'success' | 'error'; spanOp: string };
    serverError?: { type: 'success' | 'error'; spanOp: string };
    clientTrace?: { type: 'success' | 'error'; spanOp: string };
    serverTrace?: { type: 'success' | 'error'; spanOp: string };
  }>({});
  ...
  const handleClientError = async () => {
    ...
    try {
      await Sentry.startSpan({ name: 'Client Error Flow Demo', op: 'demo.client-error' }, () => {
