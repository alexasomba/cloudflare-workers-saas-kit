...
description: Durable Objects provide a powerful primitive for building stateful,
  ...
  applications.
lastUpdated: 2026-02-10T11:46:14.000Z
...
* TypeScript
  ...
  export class SeatBooking extends DurableObject<Env> {
    async bookSeat(seatId: string, userId: string): Promise<{ success: boolean; message: string }> {
      // Check if seat is already booked
      const existing = this.ctx.storage.sql
        .exec<{ user_id: string }>('SELECT user_id FROM bookings WHERE seat_id = ?', seatId)
        .toArray();
...
- JavaScript
  ...
  export class GameMatch extends DurableObject {
    ...
    async updateScore(playerId, score) {
      this.ctx.storage.sql.exec('UPDATE players SET score = ? WHERE id = ?', score, playerId);
    }
  }
  ```
  ...
  describe('ChatRoom', () => {
    ...
    it('can access instance internals and trigger alarms', async () => {
      ...
      await runInDurableObject(stub, async (instance, state) => {
        const count = state.storage.sql.exec('SELECT COUNT(*) as count FROM messages').one();
