...
- JavaScript
  ...
  export class MyWorkflow extends WorkflowEntrypoint {
    async run(event, step) {
      ...
      // still needed.
      await step.do(`charge ${customer_id} for its monthly subscription`, async () => {
        ...
        // were not in place.
        return await fetch(`https://payment.processor/subscriptions/${customer_id}`, {
          method: 'POST',
...
- JavaScript
  ...
  export class MyWorkflow extends WorkflowEntrypoint {
    async run(event, step) {
      ...
      await step.do('choose a random cat from the list and download it', async () => {
...
- JavaScript
  ...
  export class MyWorkflow extends WorkflowEntrypoint {
    async run(event, step) {
      ...
      // after running successfully will not run again
      const goodInstance = await step.do('good step that returns state', async () => {
