...
import { MagnifyingGlassIcon, MagnifyingGlassMinusIcon } from '@phosphor-icons/react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from '@workspace/components/accordion';
...
import {
  ...
  EmptyMedia,
  EmptyTitle,
} from '@workspace/components/empty';
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-3xl md:border-x">
      <div className="px-4 py-16 lg:px-6">
        <h1 className="mb-4 font-semibold text-3xl md:text-4xl">Frequently Asked Questions</h1>
        <p className="mb-8 max-w-2xl text-muted-foreground">
          Find answers to common questions about Efferd. Can&apos;t find what you&apos;re looking
      ...
      <div className="flex flex-wrap gap-1 border-b px-4 md:gap-3">
        {categories.map((cat) => (
          ...
          >
            ...
            {activeCategory === cat.id && <span className="h-0.5 w-full rounded-full bg-primary" />}
          </button>
        ))}
      </div>

      <Accordion className="space-y-2 px-4 py-12 lg:px-6" defaultValue={['1']}>
        {filtered.map((faq) => (
          <AccordionItem
